...
if (!class_exists('Dp_Regular_Menu_Module')) {
	
	class Dp_Regular_Menu_Module extends ET_Builder_Module {
		function init() {
			
			if (function_exists('et_pb_hover_options')) {
				
				$this->name       = esc_html__( 'DP Regular Menu', 'et_builder' );
				$this->slug       = 'dp_regular_menu';
					...
					'background' => array(
						'options' => array(
							'background_color'  => array(
								// Edit Default Background Color by Divi Powerful
				...
				// First Level Menu CSS By Divi Powerful
				
					'dp_first_level_menu_links' => array(
						'label'    => esc_html__( 'First Level Menu Links', 'et_builder' ),
						'selector' => '.fullwidth-menu-nav>ul>li>a',
					),
					'menu_link' => array(
				...
				// Removed Help Videos By Divi Powerful
		...
		function get_fields() {
			...
			if (function_exists('et_pb_hover_options')) {
				...
				$fields = array(
					...
					// Removed Fullwidth Menu Links By Divi Powerful
					...
					'__menu' => array(
						'type'                => 'computed',
						'computed_callback'   => array( 'Dp_Regular_Menu_Module', 'get_fullwidth_menu' ),
					...
					// Dropdown Menu Animation & Width By Divi Powerful
		...
		public function get_transition_fields_css_props() {
			...
			if (function_exists('et_pb_hover_options')) {
				...
				$fields['dropdown_menu_text_color'] = array( 'color' => '%%order_class%%.et_pb_fullwidth_menu ul li ul a' );
		...
		static function get_fullwidth_menu( $args = array() ) {
			...
			add_filter( 'wp_setup_nav_menu_item', array( 'Dp_Regular_Menu_Module', 'modify_fullwidth_menu_item' ) );

			$args = wp_parse_args( $args, $defaults );

			$menu = '<nav class="fullwidth-menu-nav et-menu-nav">';

			$menuClass = 'fullwidth-menu nav et-menu';
			...
			remove_filter( 'wp_setup_nav_menu_item', array( 'Dp_Regular_Menu_Module', 'modify_fullwidth_menu_item' ) );
		...
		function render( $attrs, $content = null, $render_slug ) {
			...
			if (function_exists('et_pb_hover_options')) {
				...
				// Dropdown Menu Width Variable by Divi Powerful
				$dropdown_menu_width      = $this->props['dropdown_menu_width'];
				...
				if ( '' !== $dropdown_menu_text_color ) {
					ET_Builder_Element::set_style( $render_slug, array(
						'selector'    => '%%order_class%%.et_pb_fullwidth_menu ul li ul a',
						'declaration' => sprintf(
							'color: %1$s !important;',
				...
				if ( et_builder_is_hover_enabled( 'dropdown_menu_text_color', $this->props ) ) {
					ET_Builder_Element::set_style( $render_slug, array(
						'selector'    => $this->add_hover_to_selectors( '%%order_class%%.et_pb_fullwidth_menu ul li ul a' ),
						'declaration' => sprintf(
							'color: %1$s !important;',
				...
				// Dropdown Menu Width By Divi Powerful
				if ( '' !== $dropdown_menu_width ) {
					ET_Builder_Element::set_style( $render_slug, array(
						'selector'    => '%%order_class%%.et_pb_fullwidth_menu .nav>li:not(.mega-menu) ul',
						'declaration' => sprintf(
							'width: %1$s ;',
							esc_html( $dropdown_menu_width )
						),
					) );
				}
				...
				if ( '' !== $dropdown_menu_width ) {
					ET_Builder_Element::set_style( $render_slug, array(
						'selector'    => '%%order_class%%.et_pb_fullwidth_menu .nav>li:not(.mega-menu) li ul',
						'declaration' => sprintf(
							'left: %1$s ;',
							esc_html( $dropdown_menu_width )
						),
				...
				if ( '' !== $dropdown_menu_width ) {
					ET_Builder_Element::set_style( $render_slug, array(
						'selector'    => '%%order_class%%.et_pb_fullwidth_menu .nav>li:not(.mega-menu).et-reverse-direction-nav li ul',
						'declaration' => sprintf(
							'right: %1$s ;',
							esc_html( $dropdown_menu_width )
						),
				...
				$this->add_classname( array(
					...
					// Add New Class by Divi Powerful
					"et_pb_fullwidth_menu",
					"free-menu-padding-small",
				) );

				// Removed Fullwidth Menu Link CSS Class by Divi Powerful

				// Edit Output by Divi Powerful
				$output = sprintf(
				...
				if ( '' !== $dropdown_menu_width ) {
					ET_Builder_Element::set_style( $render_slug, array(
						'selector'    => '%%order_class%%.et_pb_fullwidth_menu .nav li li ul',
						'declaration' => sprintf(
							'left: %1$s ;',
							esc_html( $dropdown_menu_width )
						),
