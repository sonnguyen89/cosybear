...
function ppom_hooks_render_shortcode($attr) {
	...
	if( $params['product_id'] ) {
		...
		<form class="cart" action="<?php echo esc_url( apply_filters( 'woocommerce_add_to_cart_form_action', $product->get_permalink() ) ); ?>" method="post" enctype='multipart/form-data'>
			...
			<input type="hidden" name="ppom[ppom_shorcode_product_id]" value="<?php echo esc_attr($params['product_id']);?>">
...
function ppom_hooks_check_theme_path($full_path, $template_path, $vars){
	...
	if( isset($addon_type) ) {
		$theme_path =  get_parent_theme_file_path() . "/ppom/addons/{$addon_type}.php";
	} else {
		$theme_path =  get_parent_theme_file_path() . "/ppom/{$template_path}";
	}
...
function update_converted_option_keys($new_option, $option_key, $option, $meta, $product) {
	// ppom_pa($option);
	if( !isset($meta['type']) ) return $option;
