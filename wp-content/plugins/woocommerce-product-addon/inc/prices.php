...
function ppom_price_controller($cart_item, $values) {
    ...
    // ppom_pa($cart_item);
    if( empty($cart_item) ) return $cart_item;

	if( ! isset( $values['ppom']['fields'] ) ) return $cart_item;
...
function ppom_price_get_product_base( $product_price, $product, $ppom_fields_post, 
...
function ppom_price_check_price_matrix($cart_items, $values){
	...
	if( !isset($cart_items['ppom']) ) return $cart_items;
	// ppom_pa($cart_items);
	...
	if ( ! $pricematrix_field ) return $cart_items;
	...
	foreach( $pricematrix_field as $pm ) {
	    ...
	    if( ppom_is_field_hidden_by_condition( $pm_dataname, $conditionally_hidden ) ) continue;
...
add_filter('ppom_option_price_vat', 'ppom_option_price_handle_vat', 9, 3);
function ppom_option_price_handle_vat($option_price, $meta, $product) {
	
	if( ! function_exists('wc_get_price_excluding_tax') || $option_price == '') return $option_price;
	
	if( 'yes' != ppom_get_option('ppom_taxable_option_price') ) return $option_price;
	
	if( is_cart() && apply_filters('ppom_handle_option_price_vat_in_cart', true) === true){
		$vat_type = get_option('woocommerce_tax_display_cart');
