	...
	protected function block($block) {
		...
		if (!empty($block->selectors)) {
			echo $pre . // phpcs:ignore
				implode($this->tagSeparator, $block->selectors) . // phpcs:ignore
				$this->open . $this->break; // phpcs:ignore
			$this->indentLevel++;
			$inner = $this->indentStr();
		}
		if (!empty($block->lines)) {
			$glue = $this->break.$inner;
			echo $inner . implode($glue, $block->lines); // phpcs:ignore
			if (!empty($block->children)) {
				echo $this->break; // phpcs:ignore
			}
		}
		...
		if (!empty($block->selectors)) {
			$this->indentLevel--;
			if (empty($block->children)) echo $this->break; // phpcs:ignore
			echo $pre . $this->close . $this->break; // phpcs:ignore
		}
	}
...
class titanscss_server {
	...
	protected function inputName() {
		switch (true) {
			case isset($_GET['p']): // phpcs:ignore
				return $_GET['p']; // phpcs:ignore
			case isset($_SERVER['PATH_INFO']):
				return $_SERVER['PATH_INFO']; // phpcs:ignore
			case isset($_SERVER['DOCUMENT_URI']):
				return substr($_SERVER['DOCUMENT_URI'], strlen($_SERVER['SCRIPT_NAME'])); // phpcs:ignore
		}
	}
	/**
	...
	public function serve($salt = '') {
		if ($input = $this->findInput()) {
			...
			if ($this->needsCompile($input, $output)) {
				try {
					echo $this->compile($input, $output); // phpcs:ignore
				} catch (Exception $e) {
				...
				echo file_get_contents($output); // phpcs:ignore
			}
			return;
		}
