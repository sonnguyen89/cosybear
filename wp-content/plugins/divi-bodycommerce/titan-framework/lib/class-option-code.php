...
class TitanFrameworkOptionCode extends TitanFrameworkOption {
	...
	public function printJSForPagesAndPosts() {
		...
		?>
		<script>
		<?php echo esc_html( $this->getValue( $id ) ) ?>
		</script>
		<?php
	}
	...
	public function printCSSForPagesAndPosts() {
		...
		try {
			...
			echo "<style type='text/css' media='screen'>{$css}</style>"; // phpcs:ignore
		} catch (Exception $e) {
		}
	}
	...
	public function display() {
		...
		jQuery(document).ready(function ($) {
			var container = jQuery('#<?php echo esc_html( $this->getID() ) ?>_ace_editor');
			container.width( container.parent().width() ).height( <?php echo esc_html( $this->settings['height'] ) ?> );

			var editor = ace.edit( "<?php echo esc_html( $this->getID() ) ?>_ace_editor" );
			container.css('width', 'auto');
			editor.setValue(container.siblings('textarea').val());
			editor.setTheme("ace/theme/<?php echo esc_html( $this->settings['theme'] ) ?>");
			editor.getSession().setMode('ace/mode/<?php echo esc_html( $this->settings['lang'] ) ?>');
		...
		printf( "<div id='%s_ace_editor'></div>", $this->getID() ); // phpcs:ignore
...
function registerTitanFrameworkOptionCodeControl() {
	class TitanFrameworkOptionCodeControl extends WP_Customize_Control {
		...
		public function render_content() {
			...
			<script>
			jQuery(document).ready(function ($) {
				var container = jQuery('#<?php echo esc_html( $this->id ) ?>_ace_editor');
				container.width( container.parent().width() ).height( <?php echo esc_html( $this->height ) ?> );

				var editor = ace.edit( "<?php echo esc_html( $this->id ) ?>_ace_editor" );
				container.css('width', 'auto');
				editor.setValue(container.siblings('textarea').val());
				editor.setTheme("ace/theme/<?php echo esc_html( $this->theme ) ?>");
				editor.getSession().setMode('ace/mode/<?php echo esc_html( $this->lang ) ?>');
				editor.setShowPrintMargin(false);
				editor.setHighlightActiveLine(false);
				editor.gotoLine(1);
				editor.session.setUseWorker(false);
			...
			printf( "<div id='%s_ace_editor' class='tf-code'></div>", $this->id ); // phpcs:ignore

			// The hidden textarea that will hold our contents
			?><textarea <?php $this->link(); ?> style='display: none'><?php echo esc_html( $this->value() ) ?></textarea><?php
