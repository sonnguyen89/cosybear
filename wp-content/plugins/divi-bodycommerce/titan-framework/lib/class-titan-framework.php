...
class TitanFramework {
   ...
   *
	...
	public static function displayFrameworkError( $message, $errorObject = null ) {
		...
		?>
		<div style='margin: 20px; text-align: center;'><strong><?php echo esc_html( TF_NAME ) ?> Error:</strong>
			<?php echo esc_html( $message ) ?>
			<?php
			if ( ! empty( $errorObject ) ) :
				?>
				<pre><code style="display: inline-block; padding: 10px"><?php echo print_r( $errorObject, true ) // phpcs:ignore ?></code></pre>
				<?php
			endif;
			?>
		</div>
