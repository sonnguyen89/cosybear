...
class TitanFrameworkAdminPage {
	...
	public function save_single_option( $option ) {
		...
		if ( isset( $_POST[ $this->getOptionNamespace() . '_' . $option->settings['id'] ] ) ) { // phpcs:ignore
			$value = $_POST[ $this->getOptionNamespace() . '_' . $option->settings['id'] ]; // phpcs:ignore
		} else {
			$value = '';
		}
	...
	public function saveOptions() {
		...
		if ( $_POST['action'] == 'save' ) { // phpcs:ignore
		...
		} else if ( $_POST['action'] == 'reset' ) { // phpcs:ignore
	...
	public function getActiveTab() {
		...
		foreach ( $this->tabs as $tab ) {
			if ( $tab->settings['id'] == $_GET['tab'] ) { // phpcs:ignore
				$this->activeTab = $tab;
				return $this->activeTab;
			}
		}
	...
	public function createAdminPage() {
		...
		if ( ! empty( $_GET['message'] ) ) { // phpcs:ignore
			if ( $_GET['message'] == 'saved' ) { // phpcs:ignore
				echo TitanFrameworkAdminNotification::formNotification( __( 'Settings saved.', TF_I18NDOMAIN ), esc_html( $_GET['message'] ) ); // phpcs:ignore
			} else if ( $_GET['message'] == 'reset' ) { // phpcs:ignore
				echo TitanFrameworkAdminNotification::formNotification( __( 'Settings reset to default.', TF_I18NDOMAIN ), esc_html( $_GET['message'] ) ); // phpcs:ignore
			}
		}
		...
		if ( ! empty( $activeTab ) ) {
			...
			if ( ! empty( $activeTab->settings['desc'] ) ) {
				?><p class='description'><?php echo $activeTab->settings['desc'] // phpcs:ignore ?></p><?php
			}
