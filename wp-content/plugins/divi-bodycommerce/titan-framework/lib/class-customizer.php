...
class TitanFrameworkCustomizer {
	...
	public function useCustomizerModifiedValue( $value, $postID, $option ) {
		...
		if ( ! is_array( $_POST ) ) { // phpcs:ignore
			return $value;
		}
		if ( array_key_exists( $option->getID(), $_POST ) ) { // phpcs:ignore
			return $_POST[ $option->getID() ]; // phpcs:ignore
		}

		if ( ! empty( $_POST['customized'] ) ) { // phpcs:ignore
			$customizedSettings = (array) json_decode( stripslashes( $_POST['customized'] ) ); // phpcs:ignore
			...
			wp.customize( '<?php echo esc_attr( $option->getID() ) ?>', function( v ) {
				v.bind( function( value ) {
					...
					if ( empty( $option->settings['livepreview'] ) ) {
						...
						?>
						if ( typeof localStorage !== 'undefined' ) {
							localStorage.setItem( '<?php echo esc_html( $option->getID() ) ?>', value );
						}
						...
						echo $option->settings['livepreview']; // phpcs:ignore
	...
	public function printPreviewCSS() {
		...
		if ( ! in_array( $this->owner->optionNamespace, self::$namespacesWithPrintedPreviewCSS ) ) {
			...
			echo $this->owner->cssInstance->generateCSS(); // phpcs:ignore
			echo '</style>';
		}
	}
